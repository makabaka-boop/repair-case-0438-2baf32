...
算各成员缺口。事务运行在 **Read Committed**：每条语句各自取一次语句级快照，而对
批次头的锁定读取与随后的缺口查询都发生在全部行锁到手*之后*，因此后继请求看到的
永远是前一个锁持有者已提交的状态——既包括前一个组封存/单批次封存写下的
`SEALED`，也包括排队期间刚提交的最后一片。只有所有 OPEN 成员齐备才在同一事务
里共同 `UPDATE` 为 `SEALED`；任一成员缺片则整体回滚。因此 `[A,B]` 与 `[B,A]`
的反序组请求、单批次封存与最后一片分片三方并发时只会串行等待、不会死锁，也不
可能出现"部分成员被封存"的中间态；已 `SEALED` 成员的 `sealedAt` 永不被覆盖。

这里**不能**用 Repeatable Read / Serializable：它们的事务快照固定在第一条语句
执行时，即行锁等待*之前*。后继组请求即使最终拿到了全部行锁，读到的仍是等待前的
旧快照——前一个组封存已改写的行会让它在提交时撞上 `40001 could not serialize
access due to concurrent update`（被 API 当成 500 返回）；更糟的是排队期间落库
的最后一片在旧快照里仍计为缺口，组请求会回一个与已提交状态矛盾的 `409
INCOMPLETE`，随后查询却显示该批次已齐备却仍为 `OPEN`，误导整组交接。
...
- `internal/store`：乱序、重传幂等、UTF-8 字节判同、冲突、封存不完整保持 OPEN、
  ...
  （25 轮）、反序双组与单封存竞争的确定性交错、最后一片排在组请求前面先提交时组请求
  仍须按已提交状态封存的确定性交错、关闭全部连接后重开的重启一致性；
- `internal/api`：两个独立连接池支撑的两个 HTTP 实例上交替写入/封存、全状态码矩阵、
  ...
  最后一片在组请求排队期间先落库时组请求必须返回 200 且两成员齐备的确定性交错、
  关闭并重建两个实例后的终态一致性；
